...
def main() -> int:
    ...
    parser.add_argument("--operation", choices=("refresh", "draw", "set-completions"), default="refresh")
    ...
    if args.operation != "refresh" and not args.month:
    ...
    try:
        ...
        if args.operation == "draw":
            selected = draw_presenters(payload, args.month)
            payload["assignments"].extend(
                {"employee_id": employee_id, "month": args.month}
